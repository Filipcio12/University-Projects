...
export class StudentsComponent implements OnInit {
  ...
  addToPut(firstname: string, lastname: string, email: string, telephone: string): boolean {
    firstname = firstname.trim();
    lastname = lastname.trim();
    email = email.trim();
    telephone = telephone.trim();

    const student = new Student(firstname, lastname, email, telephone);
    this.studentToPutList?.push(student);

    return false;
  }

  putAll(): void {
    this.studentService.updateStudents(this.studentToPutList)
    .subscribe({
      next: (studentList: Student[]) => {this.studentList = studentList},
      error: () => {},
      complete: () => {
        if (this.studentList != undefined) {
          this.studentService.totalItems.next(this.studentList.length);
